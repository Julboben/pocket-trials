...
export function createOverlay() {
  let lastTimer = '', toastUntil = 0, splitUntil = 0;
  ...
  function tick(now) {
    ...
    if (splitUntil && now > splitUntil) { $('split').hidden = true; splitUntil = 0; }
  }
  ...
  function setDirection(facing) {
    ...
    /** @type {HTMLElement} */ (left.querySelector('.caption')).textContent = facingRight ? 'LEAN BACK' : 'LEAN FWD';
    /** @type {HTMLElement} */ (right.querySelector('.caption')).textContent = facingRight ? 'LEAN FWD' : 'LEAN BACK';
  ...
  function showResults(result) {
    ...
    $('overlay-badge').textContent = result.official ? 'TRAIL COMPLETED' : 'CUSTOM TRAIL COMPLETED';
    $('overlay-title').textContent = record && previousBest !== null ? 'New Best!' : 'Goal Reached!';
    $('results').hidden = false;
    $('results-time').textContent = timeText(time);
    const delta = $('results-delta');
    delta.textContent = previousBest === null ? 'FIRST FINISH' : deltaText(time - previousBest);
    delta.className = 'results-delta ' + (previousBest === null ? '' : time <= previousBest ? 'ahead' : 'behind');
    const medalLabel = $('results-medal');
    medalLabel.dataset.medal = medals ? medal || 'none' : '';
    medalLabel.textContent = medals ? (medal ? medal.toUpperCase() + ' MEDAL' : 'NO MEDAL') : '';
    ...
    const targets = medals ? MEDALS.map(name => {
    ...
    const next = medals && !medal ? medals.bronze : medals && medal !== 'gold' ? medals[MEDALS[MEDALS.indexOf(medal) - 1]] : null;
    $('overlay-description').textContent = next
      ? 'Shave ' + (Math.ceil((time - next) * 10 - 1e-9) / 10).toFixed(1) + 's for the next medal.' + (result.official ? '' : ' Custom trails do not affect career progression.')
      : result.official ? '' : 'Custom trails do not affect career progression.';
    ...
    $('results-hint').textContent = 'Press ' + result.restartKey + ' to retry · Enter to continue';
    $('results-hint').hidden = false;
    $('pause-overlay').hidden = true;
    $('overlay').hidden = false;
    requestAnimationFrame(() => $('primary').focus({ preventScroll: true }));
    announce('Trail complete in ' + timeText(time) + '. All ' + result.apples + ' apples collected.'
      + (record ? ' New best time.' : '') + (rank ? ' Leaderboard rank ' + rank + '.' : '') + (medal ? ' ' + medal + ' medal.' : ''));
  }

  return { setTrail, setApples, setTimer, toast, clearToast, split, tick, announce, setDirection, showPause, hide, reopen, showResults };
}
